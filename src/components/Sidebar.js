...
export default function Sidebar() {
  ...
  const executeSprite = () => {
    ...
    for (let i = 0; i < group.length; i++) {
      ...
      let previewAreaPositionX = document
        .getElementById("previewArea")
        .getBoundingClientRect().x;
      let previewAreaPositionY = document
        .getElementById("previewArea")
      ...
      switch (group[i].id) {
        ...
        case "move10":
          ...
          if (flag) {
            ...
            catSprite.style.transform = `rotate(${rotation}deg) translate(${
              relativePositionX + xx
            }px,${relativePositionY + yy}px) `;
        ...
        case "turn15ACW":
          ...
          if (flag) {
            if (steps != 10) {
              (relativePositionX = 0), (relativePositionY = 0);
            }
            catSprite.style.transform = ` translate(${relativePositionX}px,${relativePositionY}px) rotate(${
              rotation + angle
            }deg)`;
            catSprite.style.position = "absolute";
          }
